        ...
        <div class="content">
            <div class="container-fluid">
				<div class="overlay-user-container" style="z-index:20">
					<div class="row">
						<?php
							$notes = dbquery('SELECT * FROM notes WHERE license="' . $this->userinfo['license'] . '" AND community="' . userCommunity($_SESSION['steamid']) . '"');
							if(!empty($notes)) {
								echo '
									<div class="alert alert-danger" style="border-radius: 6px;">
										<span>
											<center>
												<b>Warning!</b> This user has an active note on their profile.
											</center>
										</span>
									</div>
								';
								...
								<div class="card card-user" style="margin-top: 70px;">	
									<div class="card-body">
										<div class="author">
											...
											<?php
												if(checkPlugin('extendeduserinfo')) {
													echo '
															<hr>
														<span class="description" style="font-weight: normal;">
															Steam Hex: ' . $this->userinfo['steam'] . '
															<hr>
														<span class="description" style="font-weight: normal;">
															Discord: ' . ($this->userinfo['discord'] != false ? $this->userinfo['discord'] : "N/A") . ' 
														</span>
													';
												}
											?>
						...
						<div class="col-md-9">
							<div class="tabbable" id="tabs-125045">
								...
								<div class="tab-content">
								    ...
								    <div class="tab-pane" id="panel-note">
										<form action="<?php echo $GLOBALS['domainname']; ?>api/note" method="post" onsubmit="return submitForm($(this));">
											<div class="form-group">
					...
					<div class="row">
						<div class="col-md-12">
							...
							<table class="table table-striped">
								...
								<tbody>
									<?php 
										$warns = dbquery('SELECT * FROM warnings WHERE license="' . $this->userinfo['license'] . '" AND community="' . userCommunity($_SESSION['steamid']) . '"');
										if(empty($warns)) {
											echo '
												<tr>
													<td colspan="4">
					...
					<div class="row">
						<div class="col-md-12">
							...
							<table class="table table-striped">
								...
								<tbody>
									<?php 
										$kicks = dbquery('SELECT * FROM kicks WHERE license="' . $this->userinfo['license'] . '" AND community="' . userCommunity($_SESSION['steamid']) . '"');
										if(empty($kicks)) {
											echo '
												<tr>
													<td colspan="4">
					...
					<div class="row">
						<div class="col-md-12">
							...
							<table class="table table-striped">
								...
								<tbody>
									<?php
										$bans = dbquery('SELECT * FROM bans WHERE identifier="' . $this->userinfo['license'] . '" AND community="' . userCommunity($_SESSION['steamid']) . '"');
										if(empty($bans)) {
											echo '
												<tr>
													<td colspan="5">
					...
					<div class="row">
						<div class="col-md-12">
							...
							<table class="table table-striped">
								...
								<tbody>
									<?php 
										$commendations = dbquery('SELECT * FROM commend WHERE license="' . $this->userinfo['license'] . '" AND community="' . userCommunity($_SESSION['steamid']) . '"');
											...
											foreach($notes as $note) {
												echo '
													<tr>
														...
														<td>
															' . date("m/d/Y h:i A", $note['time']) . '
														</td>
												';
													if(hasPermission($_SESSION['steamid'], 'delrecord')) {
														echo '
														<form action="'.$GLOBALS['domainname'].'api/delnote" method="post" onsubmit="return submitForm($(this));">
															<input type="hidden" name="noteid" value="'.$note['ID'].'" />
															<input type="submit" id="remove-note-'.$note['ID'].'" style="display: none;" />
															<td class="table-remove-button"><span class="label label-danger" onclick=\'$("#remove-note-'.$note['ID'].'").click();\' style="cursor: pointer;">Remove</span></td>
														</form>
														';
													} else {
														echo '<td></td>';
								...
								<div class="col-md-12">
									...
									<table class="table table-striped">
										...
										<tbody>
											<?php 	
												$hasresult = false;											
												foreach(dbquery('SELECT * FROM warnings WHERE community !="' . escapestring(userCommunity($_SESSION['steamid'])) . '" AND license="' . $this->userinfo['license'] . '"') as $result) {
													echo '
														<tr>
															<td>
																Warning
															...
															<td>
																' . date("m/d/Y h:i A", $result['time']) . '
															</td>
														</tr>
													';
													$hasresult = true;
												}									
												foreach(dbquery('SELECT * FROM kicks WHERE community !="' . escapestring(userCommunity($_SESSION['steamid'])) . '" AND license="' . $this->userinfo['license'] . '"') as $result) {
													echo '
														<tr>
															<td>
																Kick
												...
												foreach(dbquery('SELECT * FROM bans WHERE community !="' . escapestring(userCommunity($_SESSION['steamid'])) . '" AND license="' . $this->userinfo['license'] . '"') as $result) {
													echo '
														<tr>
															<td>
																Ban
															...
															<td>
																' . date("m/d/Y h:i A", $result['ban_issued']) . '
															</td>
														</tr>
													';
													$hasresult = true;
												}				
												foreach(dbquery('SELECT * FROM commend WHERE community !="' . escapestring(userCommunity($_SESSION['steamid'])) . '" AND license="' . $this->userinfo['license'] . '"') as $result) {
													echo '
														<tr>
															<td>
																Commendation
												...
												if(!$hasresult) {
													echo '
														<tr>
															<td colspan="3"><center>No Global Record Found</center></td>
														</tr>
													';
												}
											?>
